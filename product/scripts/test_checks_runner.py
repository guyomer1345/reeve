...
def test_fix_never_hands_the_GENERATED_CODE_MAP_to_a_formatter(tmp_path):
    """The same defect one directory over, found by the cold-context reviewer on a real drive
    and reproduced from scratch. The first filter listed `.workflow/` and `.claude/`, which
    reads as "the loop's own files" and is a SMALLER set than the loop's own ARTIFACTS:
    `docs/knowledge/graph.json` is generated by `.workflow/codemap.sh`, lands in an item's
    `files_touched` (stack-wiring regenerates it on every new greenfield project), and lives
    under `project_root` because it ships with the product's docs. `ruff format` rewrote it
    with magic trailing commas -> invalid JSON -> `check_codemap_fresh.py` refuses the commit
    with "code map is stale". Both spellings: `project_root` is `./project` on greenfield and
    `.` on brownfield."""
    root = _project(tmp_path, "")
    logger = root / "fixlog.sh"
    logger.write_text('#!/usr/bin/env bash\nprintf "%s\\n" "$@" >> "$PWD/fixed.txt"\n')
    ...
    (root / ".workflow" / "checks.env").write_text(f'FMT_FIX="bash {logger}"\n')
    r = _run(root, "--fix",
             "project/src/a.py",                          # real source — must go through
             "project/docs/knowledge/graph.json",         # greenfield layout
             "./docs/knowledge/graph.json",               # brownfield layout
             "project/docs/knowledge/src/a.py.md")        # a knowledge NODE, also not source
    assert r.returncode == 0, r.stderr
    assert (root / "fixed.txt").read_text().split() == ["project/src/a.py"], \
        "the code map reached the formatter"
    assert "graph.json" in r.stderr              # named, never silently dropped


def test_a_path_merely_CONTAINING_knowledge_is_still_formatted(tmp_path):
    """The withhold is `docs/knowledge/`, not the word `knowledge` — a project with its own
    `src/knowledge/` module is stack source and must still be fixed. Guards the over-correction,
    which is the failure mode a denylist repair invites."""
    root = _project(tmp_path, "")
    logger = root / "fixlog.sh"
    logger.write_text('#!/usr/bin/env bash\nprintf "%s\\n" "$@" >> "$PWD/fixed.txt"\n')
    logger.chmod(0o755)
    (root / ".workflow" / "checks.env").write_text(f'FMT_FIX="bash {logger}"\n')
    r = _run(root, "--fix", "project/src/knowledge/store.py", "project/knowledge_base.py")
    assert r.returncode == 0, r.stderr
    assert (root / "fixed.txt").read_text().split() == [
        "project/src/knowledge/store.py", "project/knowledge_base.py"]
