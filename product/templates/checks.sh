...
case "$MODE" in
  --fix)
    ...
    #
    # The loop's OWN ARTIFACTS are withheld from the fixers, and this is a repair, not a
    ...
    # formatter has no business in a loop artifact for ANY stack, so the filter is
    # stack-agnostic and lives here rather than in per-project config. Skipped paths are
    # named, never silent.
    #
    # `docs/knowledge/` IS SUCH AN ARTIFACT AND IT DOES NOT LIVE UNDER `.workflow/`. The first
    # version of this filter listed the two runtime directories, which read as "the loop's own
    # files" and is not the same set: the code map `docs/knowledge/graph.json` is GENERATED by
    # `.workflow/codemap.sh`, is routinely in an item's `files_touched` (the stack-wiring
    # transition regenerates it on every new greenfield project), and sits under `project_root`
    # because it ships with the product's docs. It went to the fixers. MEASURED, the same way
    # and with the same outcome: `ruff format` rewrites it with magic trailing commas, the map
    # comes back as invalid JSON, and `check_codemap_fresh.py` then refuses the commit with
    # "code map is stale — missing or unreadable". Both `<proot>/docs/knowledge/` spellings are
    # matched because `project_root` is `./project` on greenfield and `.` on brownfield.
    # The lesson, stated so the next artifact is not found the same way: the hazard is a file
    ...
    if [ -n "${STACK_GATE_NONE:-}" ]; then
      ...
      for f in "$@"; do
        case "${f#./}" in
          ...
          docs/knowledge/*|*/docs/knowledge/*) withheld+=("$f") ;;
          *)                                   fixable+=("$f") ;;
        esac
      done
      [ "${#withheld[@]}" -gt 0 ] && echo "  (not passed to the fixers — loop artifacts," \
        "which a code formatter would corrupt: ${withheld[*]})" >&2
