...
2. **Scaffold the workflow layout** (provisional — EXPAND):
   ...
   .workflow/
     ...
     control.json      # the loop's PAUSE latch — written by `drain.py record`, read by the session driver — RUNTIME, add to .gitignore
     ...
     monitor.json      # the drive heartbeat's last observation — is the loop still moving, was it
                       # nudged, did it escalate — RUNTIME, add to .gitignore
     ...
     worker-budget/    # RUNTIME — one breadcrumb per exit of the worker-budget hook, so the
                       # question "has that mechanism ever actually run?" is answerable from the
                       # tree instead of assumed; written on EVERY tool call, add to .gitignore
