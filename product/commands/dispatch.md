...
**And it can now be automatic.** `loop.sh --supervise` (inside tmux) runs a poller beside this session that sends `/clear` then `continue` for you once `clear_safe` holds — the anchor written, no checkpoint parked, no dialog open, **and the session idle**. That last one is why a reset can lag the anchor by a minute or two: the anchor lands mid-turn, and typing into a running turn puts text in the prompt box instead of starting one. It never writes the handoff: that stays yours, because only this session knows what a complete one says.
