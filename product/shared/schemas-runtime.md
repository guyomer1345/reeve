...
> **The unattended drive's governors moved out** — [`schemas-drive.md`](schemas-drive.md): `context.json` ·
> `handoff-gate.json` · `turn-gate.json` · `monitor.json` · `awaiting-input.json` · `session-idle.json` ·
> `in-flight/` · `supervise-latch.json`. They are one mechanism (reading → band → gates → heartbeat →
> supervisor), they are where this file's growth came from, and they answer a different question from
> everything left here: *may this session keep going, be reset, or be typed at?*
