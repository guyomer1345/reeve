...
learns to ignore.
- `request` — `{ kind: demo|qa|setup|reconcile|forecast|steer|spec, what, expected, how?(←setup-guide), tasks?[], blocking, token }`. **`blocking: false` is reachable only for a deferred `qa`** — the item has already documented, committed and closed, so the ticket holds nothing: neither the turn ladder nor the context-reset gate counts it. Every other kind is *answer before proceeding*, and **absent reads as blocking** — a record nobody can read must not release a gate (`bus.ticket_blocks`).
