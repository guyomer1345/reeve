...
if is_git commit; then
  ...
  # `.workflow/items/<id>/verify-verdict.md`, first line exactly `pass: true|false` — or, for a
  # non-item motion (no verify BY DESIGN), a staged `.workflow/maintenance/<id>.json` receipt
  # naming which motion it is (shared/schemas.md · commit-receipt).
