...
5. **Track provisional debt on the no-demo path:** at spec-completion, if the sandbox gate will not fire (no
   demo for this item — the gate's conditions live in `create-demo`'s *sandbox gate* section, the single
   source, and are applied by whoever routes),
   spawn a `create-issue` (kind=debt) for each field left `provisional`, so no deferred decision is silently
   lost. (When a demo does run, the demo-approval route files these instead — exactly one path files, never
   both.)
