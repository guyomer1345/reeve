...
the section name is the anchor; the drive governors split again to `shared/schemas-drive.md` at D246, on the same
rule).
...
<launch root>      # where Claude runs = orchestrator home (process / machinery)
  ...
  .workflow/
    ...
    orchestrator.lock # RUNTIME — the single-orchestrator LIVENESS marker (distinct from bus.lock, the DAEMON's election). Held for a session's lifetime by an ORCHESTRATOR launch — a human via the loop.sh launcher, a runner-launched `claude -p` via `flock` itself — and PROBED by the daemon's runner as its spawn precondition, so the runner never launches a duplicate alongside a live orchestrator. The kernel drops it on death, so it never goes stale (unlike a pidfile) — D123, gitignored · bus:write (the daemon's runner probes it and a runner-spawned `flock` child holds it; NOT bus:none — the daemon does touch it) · pin (co-location with the runtime tree; flock is sound on 9p, so correctness does NOT require it — D115/D123)
    ...
    context.json    # RUNTIME — the session's last CONTEXT READING {used, window, mono}, published by `statusline.py` every turn and read by `context_band.py`. The one crossing of a real wall: the statusline is the ONLY surface Claude Code exposes a token count to (hooks and the model get none), so it can SEE and not act while the loop can ACT and not see. Until this existed nothing crossed — the banner went to a human's eyes and that was the whole channel — D206, gitignored · bus:none · pin
    handoff-gate.json # RUNTIME — the context gate's latch {armed_handoff_mtime, demands}: the mtime `handoff.md` had when the band ENTERED `handoff-now`, and how many times the `Stop` hook has demanded an anchor since. Gives "freshly written" a moment to be fresh RELATIVE TO — a TTL would have been a guess. Disarmed by the first reading that is not `handoff-now` (in practice the turn after a /clear), which makes the demand once-per-fill rather than a nag. Deliberately NOT pinned: a project whose runtime half is gone still needs its anchor written, so nothing on this path may resolve runtime.json — D215, gitignored · bus:none · no-pin (the repo mount ON PURPOSE: nothing on this path may have to resolve runtime.json)
    monitor.json    # RUNTIME — the drive heartbeat {state,action,why,fingerprint,pulse,counters}, written by `scripts/monitor.py` from `supervise.sh` and read by `bus.py`'s steer floor. Exists for the one failure a `Stop` hook CANNOT see — the session that never ends a turn (it idles, or sits in a dialog, D217's finding). The pulse is the newest write the LOOP made — an ALLOW-LIST, not all of `.workflow/`, because the supervisor's own `--gate` call writes there and a monitor watching the whole directory would see its own heartbeat reflected back off a dead session (found by a test, not by reasoning). Needs no new hook: the loop writes state.json every iteration and a worker-budget breadcrumb on every tool call, so "the loop has written nothing for 10m" is an observation, not a guess. WAITING (parked · dialog · paused) is never stalling. One nudge, then a `steer` park — the away channel only alerts on checkpoints, so escalation goes through the machinery that owns it — D225/ask#12, gitignored · bus:none · no-pin
    turn-gate.json  # RUNTIME — the turn gate's cross-turn memory {fingerprint,satisfied,demands,rung}, written by `hooks/turn_gate.py` and judged by `scripts/turn_check.py`. Exists because two of the three questions a turn-end gate asks are only answerable ACROSS turns: *did this turn move anything* needs the fingerprint the previous stop ended on (drive.py's, reused so the driver and the gate share one definition of motion), and *has the human already seen this report* needs the digest last satisfied — without which an unchanged loop is asked for the same block every turn, which is the nuisance that gets a gate switched off. Written on every stop INCLUDING the blocking ones, or the first block poisons the second's comparison — D224/ask#11+#12, gitignored · bus:none · no-pin
    wave-decision.json # RUNTIME — the dispatch boundary's recorded answer to "what else could run right now?" {decided_at,head,batch,considered,…}, written ONLY by `check_wave_independence.py --record` and read by the PreToolUse dispatch guard, which REFUSES an `execute` no verdict at this HEAD covers. Exists because D192's "never wait alone" was a sentence: the verdict was computed, read by a human, and thrown away, so a router that blocked alone violated nothing — D215/ask#6, gitignored · bus:none · no-pin
    awaiting-input.json # RUNTIME — "a dialog is open" {kind,session_id}, written by the Notification hook when Claude Code raises a permission/elicitation prompt and removed by the Stop hook (a turn that ended cannot be in a dialog, so no TTL). The THIRD `clear_safe` blocker, found by probing rather than reasoning: a live probe drove a real session into a permission prompt where it sat, invisible to parked/. `idle_prompt` is deliberately excluded — it is the supervisor's own trigger — D217, gitignored · bus:none · no-pin
    session-idle.json # RUNTIME — "the session is sitting at an idle prompt" {kind,session_id}, written by the SAME Notification hook on `idle_prompt` (the one kind the dialog flag must ignore) AND by `hooks/session_start.py` on startup/resume/clear (a started or cleared session is idle BY DEFINITION and the harness can never say so — its idle timer arms off the last message timestamp, and a cleared session has none; `compact` is excluded because auto-compact fires mid-turn), removed by `hooks/prompt_submit.py` (UserPromptSubmit) alone — D242. The FOURTH `clear_safe` condition and the only one in the POSITIVE — absent reads as not-idle. It is the one the other three were wrong about: `handoff.md` is written DURING a turn, so all three went true while the model was still working and every reset the supervisor ever sent went into a running turn. Keys sent then are not queued into it — they land in the prompt box as text, never submitted, and the next poll adds more (OBSERVED 2026-09-19: `/clear continue /clear continue` stacked, and the Esc that flushed it ran the `/clear` with no `continue`). A BRACKET, not a timer: the harness says when idleness begins, a submitted prompt is the only thing that ends it. Presence is the fact, so a torn file still counts as idle — D239, gitignored · bus:none · no-pin
    in-flight/      # RUNTIME — one file per dispatched worker that has not returned {agent,description,session_id}, keyed by `tool_use_id`. Written by `dispatch_guard.py` (PreToolUse on Agent|Task) and removed by `dispatch_return.py` (PostToolUse on the same) — BOTH HOOKS ALREADY EXISTED AND WERE ALREADY WIRED; the fact was observable the whole time and nothing read it. The turn gate and the reset gate were both written when a dispatch BLOCKED the parent, so a turn that ended meant a session that had stopped; the harness auto-backgrounds agents now. Without this, `may_end` called every background dispatch a stop for nothing (`demands: 35` against MAX_DEMANDS 2 on one real drive, all false — and the gate then stood down for the real case), and `clear_safe` would have cleared a session mid-dispatch because `idle_prompt` fires while a worker runs. Presence is the fact; a torn file still counts. Cleared wholesale at SessionStart; a 1h age-out is the within-session backstop only — D241, gitignored · bus:none · no-pin
    supervise-latch.json # RUNTIME — the supervisor's attempt ledger {attempts,used,gave_up}. `gave_up` is a fact about that process; `monitor.py` reads it and parks the `steer` (D246) — before that the give-up reached `supervise.log` and nothing else. `send-keys` exiting 0 means tmux took the keystroke, never that the TUI submitted it, so a send that does not land changes nothing the gate reads and the gate fires again every poll, forever — the only actuator in the package with no give-up, beside RUNNER_MAX_ATTEMPTS / MAX_DEMANDS / STALL_LIMIT. Graded on the DERIVED effect (a landed `/clear` collapses the context reading), retired on every tick so three SUCCESSFUL resets cannot trip a cap meant for three failed ones — D239, gitignored · bus:none · no-pin
    control.json    # RUNTIME — the loop's PAUSE latch {paused,at,by}, written MECHANICALLY by `drain.py record` when a `control` pause/resume is applied and read by the session driver before it spawns. Exists because `pause` had NO durable representation at all: the op was validated and delivered, then honoured only by whichever session read it, so it expired at that session's exit — the precise moment an unattended driver decides whether to start another one. Sits on the drain's ARITHMETIC side (a flag), unlike `reprioritize` which is judgment and stays the orchestrator's — D201, gitignored · bus:none (the drain writes it and the driver reads it; the daemon does not) · pin (co-location with the runtime tree)
    worker-budget/<outcome>.json # RUNTIME — one file per EXIT of `worker_budget.py` {outcome,count,payload_keys,…}: `located` (it read a real worker's own transcript), `no-transcript`, `no-usage`, `no-agent-id`. Exists because that hook fails silent on every path AND one of those paths is its normal exit — it is registered on PostToolUse with no matcher, so the orchestrator's own tool calls take the `no-agent-id` branch — which made a dead trigger and a healthy loop produce identical evidence: none. Ask #3 was ticked off against a mechanism nobody could show had ever executed. ONE FILE PER OUTCOME so a parallel wave cannot have one worker clobber another's outcome; `count` is advisory (an increment can be lost) and EXISTENCE is the signal, which cannot be. Written on every tool call, so it is gitignored for churn as much as for being runtime — D222 (which also owns the two locator defects building it found), gitignored · bus:none · no-pin (written from inside a worker, which must never have to resolve runtime.json)
    ...
    inbox/          # RUNTIME — append-only TYPED command queue (verdict|intake|control|release|question) the bus writes; matched at boundaries — D90/D91/D93/D105, gitignored · bus:write · pin
    thread/         # RUNTIME — the console conversation: thread.json (the human's questions + the project's prose answers, one rolling thread) and handoff.md (written at rotation, when the estimated context crosses config.thread.rotate_at_tokens; it carries only what is NOT re-derivable — the human's turns verbatim, open threads, pointers to outcomes that landed with an owner — and NO project prose answer, because rotation destroys its own source and a restated answer would be laundered into durable memory: schemas.md § conversation-thread, D172). Single-writer (the `answer` skill), the bus only reads it for the Conversation panel — the mirror of outbox/. NOT committed: a transcript would be a second copy of every decision it contains (the spec/backlog/decision record already own those), and the session Claude actually resumes is machine-local and cwd-keyed, so committing buys a readable log and not a resumable conversation, gitignored · bus:read · pin (it crosses the process boundary, exactly like outbox/ and parked/)
    ...
    forecasts/<id>.json # the chain-forecast for a change: the loop's prediction of its OWN routing, frozen on approval and then the anchor reality is measured against. COMMITTED and not runtime, because it must survive a cold start and a /rebind to a machine where the runtime tree explicitly may not — and safe to commit because it carries credential key NAMES only (a linted invariant, forecast.py). It CANNOT live in the parked record: unpark deletes that at the instant of approval, which is exactly when the forecast must be frozen. Item-dir lifecycle — pruned by the audit pass on the same promoted.json marker that closes items/<id>/ — D159/D162  (committed) · bus:read (the console's Forecast-chains panel)
