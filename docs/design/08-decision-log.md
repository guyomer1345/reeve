...
## D136 — Interactive context governor: a statusline budget-warning → `/dispatch` handoff → `/clear` → `SessionStart` auto-rehydrate, with a `PreCompact` backstop — the manual-clear cycle made a first-class, hook-backed mechanism for non-runner sessions **[DECIDED — extends D92/D131/D134 to interactive sessions; from the second planning pass on the lived onboarding (2026-07-26); build pending Phase 6 (`11`)]**
