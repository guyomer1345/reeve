...
## D238 — the three defects the first post-`D236` run exposed: a formatter loose in the knowledge base, an anchor nobody asked for, and a harness that called a promoted item a failure **[BUILT 2026-09-18 — 1,637 tests, 9 meta-gates. All three found by ONE run; none was reachable by reading]**
The full run came back **26 steps, 4 failures**, and the four resolved to three causes. None was a regression
from `D233`–`D237`; two were pre-existing and one was created by `D236` itself. All three are fixed here.

**1. `checks.sh --fix` hands the GENERATED CODE MAP to the stack formatter — found by the cold-context reviewer
on its first real drive, and it is `D180`'s own defect one directory over.** `D180` fixed this hazard after
`ruff format` corrupted a `promises.json`, and scoped the filter to `.workflow/*` and `.claude/*` — *"the loop's
own runtime"*. That is a **smaller set than the loop's own ARTIFACTS**:
`<project_root>/docs/knowledge/graph.json` is generated by `.workflow/codemap.sh`, is routinely in an item's
`files_touched` (stack-wiring regenerates it on **every new greenfield project**), and lives under
`project_root` because it ships with the product's docs. Reproduced from scratch rather than taken on the
reviewer's word: `ruff format` rewrites it with magic trailing commas, `json.load` raises *"Illegal trailing
comma before end of object"*, and `check_codemap_fresh.py` then refuses the commit with *"code map is stale —
missing or unreadable"*. Both `<proot>/docs/knowledge/` spellings are now withheld, because `project_root` is
`./project` on greenfield and `.` on brownfield.
**The lesson is written into the comment so the next artifact is not found the same way: the hazard is a file
the LOOP GENERATES, not a file in a particular directory.**
**Rejected — withholding by extension (`*.json`, `*.md`).** A project may legitimately format its own data
files, and the runner is stack-agnostic — it cannot know which extensions the stack's formatter owns.
**Guarded against the over-correction** with its own test: `src/knowledge/store.py` is stack source and must
still be fixed. A denylist repair invites exactly that mistake, which is how this defect existed at all.

**2. The anchor is demanded at a clean turn end, and a session that ends UNCLEAN is never asked.** Both modes
failed `resume anchor written`, both having written prose where the sha goes
(`base_sha: (the tech-stack commit)`). The shipped predicate is not broken — `context_band.anchor_names_base`
and the harness's regex are **byte-identical** and both correctly reject it. It never got to speak: the ladder
returns its **first** demand, so a turn owing rung 1 never hears rung 2, and `turn_gate.py` gives up after
`MAX_DEMANDS` — which is precisely the moment the session ends anyway, leaving a successor with no
`git log <base_sha>..HEAD` to run. So the give-up now **converts to the anchor demand once** instead of
releasing silently. This is the same shape as `D234`'s hole one rung down: a demand suppressed by the rung
above it, on the path where it mattered most.
**THE FIRST VERSION OF THIS FIX WAS WRONG AND THE WEDGE TEST CAUGHT IT**, which is the entry's own small lesson.
Setting `rung = "anchor"` does not work: the **ladder still returns `continue`** on the next stop (rung 1 being
unsatisfied is *why* we are at the give-up), so the rung alternated `continue → anchor → continue`, `same_rung`
was never true, the counter restarted every time, and the session was blocked **forever** — a wedge, in the one
path built to prevent wedging. It is a latch **flag**, not a rung: spent once, cleared as soon as the anchor is
good so a later bad one can demand again. Three tests, and the wedge control is the one that earns its place.

**3. The harness called a promoted item a failure, because a timeout short-circuited the question.**
`an_item_actually_completed` reads `promoted.json` — the package's own finished marker — and it was gated behind
the drive's **exit status**. Greenfield promoted `tech-stack` and was then killed at the timeout while writing
the next thing, so the seam reported *"nothing went round"* about an item carrying the marker. **A kill does not
un-promote.** Promotion is now the authority and the exit status is reported beside it, never in front of it:
the line reads *"tech-stack promoted / timed out after 3600s — still writing when it was killed"* — the pass and
the caveat together, neither hidden. Verified by replaying the helper against the very tree that failed.
**And the window itself moved, 3600/2700 → 5400/4200**, because `D236` changed the loop's shape: a gating review
finding sends the item back through refine → planner → execute → verify → review, and that round **measured 23
minutes** on top of an item already built once. A budget set before that node existed reports a red on every
drive unlucky enough to find a real defect — which is the one case the harness must not punish.

**What the run PROVED, recorded because a failure count reads as a bad run and this was not one.** `D236` ran
end to end on its first outing: `gating: true` → `plan-delta-01` → re-plan → re-execute → `pass: true` →
`gating: false` → `promoted.json`, converging in **one round** and finding defect 1 on the way. `D233`'s seam
half went green on brownfield (*"goal G-001, in HEAD"*). And the brownfield session **refused, in its own
words**, to stamp `locked` on an unconfirmed spec (`D235`'s rule) and to write a `planner:decompose` receipt for
a node that had not run (`D233`'s new kind not abused) — the two things most worth worrying about in what this
session shipped, declined unprompted.

*Evidence:* the run's four failures, each traced to its cause above; `ruff format` reproduced on a synthetic
code map; the anchor predicates compared directly; the seam replayed on `greenfield-ao32t8e1`.
**Builds on:** **D180** (the filter this widens, and the defect it repeats), **D236** (the reviewer that found
defect 1, and the cost that moved the window), **D234** (the same suppressed-demand shape one rung up),
**D231** (`MAX_DEMANDS`, which the anchor conversion had to not break), **D220**/**D227** (why the receipt is
