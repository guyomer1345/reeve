...
- **Base commit:** `c0b3272` (the post-Phase-7 register, 2026-08-02). The surface was **read** at `9e4dcb6`; the
  audit was then **rebased onto `7861cd1`** before landing, picking up two commits made in the main checkout while
  it ran: `03b0e61` (**D177** — the governor's `warn_pct` default 75→30) and `7861cd1` (two drive-found legibility
  defects appended to `07`). **D177 is therefore NOT in this audit's scope** — the surface was read at D176.
  Neither commit changes a finding below; both are accounted for in the figures, which were re-measured after the
  rebase. *(D177's own finding — a detector that fires and tells nobody — is the same class as JF1 below: a
  mechanism reporting success while the human learns nothing. Noted, not audited.)*
...
| A4 | `07-open-questions.md:684` | The register's self-measurement — "**21.4k**, ~3.6k of headroom" — never learned the D176 section appended below it. | Refreshed to **23.2k estimated / ~1.8k headroom** (74.4k chars, *after* the `7861cd1` rebase) and cross-pointed to JF1, since the *estimate* is now the thing in doubt rather than the headroom. **The line records that it was written at 22.3k one day earlier and was stale before it merged** — JF3 stated as a fact rather than a prediction. |
...
*20 skills + 2 agents; max D177 post-rebase* · `check_enum_coherence` *4 enum, 1 registry, 3 layout* ·
`build-release` *63 shipped files, 22 install entries* · `check_contracts` *0 advisory*). **Full suite: 853
passed** — identical to the pre-audit baseline and re-run after the rebase, so it also covers D177's
`statusline.py` change. These are prose edits, and the unchanged suite is the proof they changed no behaviour.
...
- **Measured, this session, against the tokenizer that actually enforces the ceiling:**
  - `07-open-questions.md` — **71,287 chars → 26,879 tokens** (the Read tool reported the count as it paged) =
    **2.652 chars/token**. The gate scored it **22,277**: green, ~2.7k of apparent headroom, and unreadable.
    *(That file has since grown to 74,350 chars — **~28.0k real against a 23.2k estimate**, so the gap widened
    from ~4.6k to ~4.8k tokens in one day. The ratio is what matters and it is stable; the absolute numbers move.)*
